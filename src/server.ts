...
app.post(
  ...
  requireAuth,
  requireAdminOrGestor,   // mantenha só este gate (ou o seu canUserPostOnOperation, mas não os dois)
  csrfProtection,
  async (req: Request, res: Response) => {
    const user = (req.session as any).user;
    const operacao_id = Number(req.params.id);
...
// ———————————————————————————————————————————————
async function buildMonitorData(id: number) {
  const op = await db('operacoes').where({ id }).first();
  ...
  const k = await db('operacao_eventos as e')
    ...
    .sum<{ detidos: any  }>({ detidos:  db.raw('COALESCE(f.pessoas_detidas_qtd,0)') })
    .first();
  ...
  const seriesPorCidade = await db('operacao_cidades as oc')
    ...
    .select(
      ...
      db.raw('COALESCE(sf.det_sum,0)            as detidos'),
  ...
  const efetivoByCity = await db('operacao_cidades as oc')
    ...
    .select(
      'c.id as cidade_id',
      'c.nome as cidade',
      db.raw('COALESCE(SUM(e.total_agentes), 0)   as agentes'),
      db.raw('COALESCE(SUM(e.total_viaturas), 0)  as viaturas'),
      db.raw('COALESCE(SUM(e.pc_agentes), 0)      as pc_agentes'),
      db.raw('COALESCE(SUM(e.pc_viaturas), 0)     as pc_viaturas'),
      db.raw('COALESCE(SUM(e.pm_agentes), 0)      as pm_agentes'),
      db.raw('COALESCE(SUM(e.pm_viaturas), 0)     as pm_viaturas'),
      db.raw('COALESCE(SUM(e.outros_agentes), 0)  as outros_agentes'),
      db.raw('COALESCE(SUM(e.outros_viaturas), 0) as outros_viaturas')
    )
    .orderBy('c.nome');
...
app.get('/operacoes/:opId/fiscalizacoes/:fiscId/editar',
  requireAuth, csrfProtection, async (req, res) => {
    ...
    const aprs = await db('evento_apreensao as ea')
      ...
      .select(
        { id: 'ea.evento_id' },           // mantém apr.id funcionando no EJS
        'ea.tipo',
        'ea.quantidade',
        'ea.unidade',
        { obs: 'oe.obs' }                 // obs vem do evento pai da apreensão
      );
...
app.get('/operacoes/:opId/apreensoes/:eventoId/editar', requireAuth, async (req, res) => {
  const { opId, eventoId } = req.params as any;
...
app.post('/operacoes/:opId/apreensoes/:eventoId/editar',
  ...
  async (req: Request, res: Response) => {
    ...
    const fiscOk = await db('operacao_eventos')
      ...
      .first();
    if (!fiscOk) return res.status(400).send('Fiscalização inválida para esta operação.');
...
async function buildRelatoriosData(filters: ReportFilters) {
  ...
  const fiscList = await applyCommonWhere(
    ...
    .select(
      ...
      db.raw("COALESCE(json_array_length(itens_por_fis.itens), 0) as itens_apreendidos")
    )
    .orderBy('e.ts', 'desc');
...
type ReportFilters = {
  from?: string;     // 'YYYY-MM-DD'
  to?: string;       // 'YYYY-MM-DD'
  opId?: number;     // operação (obrigatória no front)
  cidadeId?: number; // cidade (opcional)
};
...
async function buildRelatoriosData(filters: ReportFilters) {
  ...
  const kpis = await baseFis.clone().select(
    db.raw('COUNT(e.id)                                   AS fiscalizacoes'),
    db.raw('COALESCE(SUM(f.pessoas_abordadas), 0)         AS pessoas'),
    db.raw('COALESCE(SUM(f.veiculos_abordados), 0)        AS veiculos'),
    db.raw('COALESCE(SUM(f.pessoas_detidas_qtd), 0)       AS detidos'),
