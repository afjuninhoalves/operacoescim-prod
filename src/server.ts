...
app.get(
  '/operacoes/:opId/fiscalizacoes/:eventoId/editar',
  requireAuth,
  csrfProtection,
  async (req, res) => {
    const user = (req.session as any).user;
    ...
    const base = await db('operacao_eventos')
      ...
      .first();
    if (!base) return res.status(404).send('Fiscalização não encontrada.');
...
app.post('/operacoes/:opId/fiscalizacoes/:eventoId/editar',
  ...
  async (req, res) => {
    ...
    const toInt  = (v:any) => { if (v === '' || v == null) return 0; const n=Number(v); return Number.isFinite(n)&&n>=0?Math.floor(n):0; };
    const toBool = (v:any) => v === 'on' || v === 'true' || v === '1';
    const s      = (v:any) => (String(v||'').trim() || null);

    // Fiscalização (obriga tipo_local)
    const tipo_local = String(req.body.tipo_local || '').trim();
    if (!tipo_local) return res.status(400).send('Tipo de local é obrigatório.');
    ...
    let items:any[] = [];
    try { items = JSON.parse(req.body.apreensoes_json || '[]'); } catch {}

    // helper para criar evento dentro da mesma transação
    async function createEventoBaseTrx(trx:any, p: {operacao_id:number; cidade_id:number; user_id:number; tipo:string; obs:any}) {
    ...
    await db.transaction(async (trx) => {
      ...
      const existentes: number[] = await trx('evento_apreensao')
      ...
      for (const it of items) {
        ...
        const quantidade =
          it?.quantidade === '' || it?.quantidade == null
            ? null
