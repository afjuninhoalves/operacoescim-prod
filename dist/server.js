...
function applyCommonWhere(q, f) {
    if (f.from)
        q.where('e.ts', '>=', new Date(`${f.from}T00:00:00Z`));
    if (f.to)
        q.where('e.ts', '<', new Date(`${f.to}T23:59:59.999Z`));
    if (f.opId)
        q.where('e.operacao_id', f.opId);
    if (f.cidadeId)
        q.where('e.cidade_id', f.cidadeId);
    return q;
}
async function buildRelatoriosData(filters) {
    ...
    const baseFis = applyCommonWhere(db('operacao_eventos as e')
        .join('evento_fiscalizacao as f', 'f.evento_id', 'e.id')
        .where('e.tipo', 'fiscalizacao'), filters);
    ...
    const aprAgg = await applyCommonWhere(db('operacao_eventos as fe') // fiscalização (evento pai)
        .leftJoin('evento_apreensao as a', 'a.fiscalizacao_evento_id', 'fe.id')
        .where('fe.tipo', 'fiscalizacao'), filters)
    ...
    const topLocais = await applyCommonWhere(db('operacao_eventos as e')
        ...
        .whereNotNull('f.local_nome'), filters)
