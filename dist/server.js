...
async function buildRelatoriosData(filters) {
    ...
    const kpis = await baseFis
        .clone()
        .select(db.raw('COUNT(e.id)                                         AS fiscalizacoes'), db.raw('COALESCE(SUM(f.pessoas_abordadas), 0)               AS pessoas'), db.raw('COALESCE(SUM(f.veiculos_abordados), 0)              AS veiculos'), db.raw('COALESCE(SUM(f.pessoas_detidas_qtd), 0)             AS detidos'), db.raw('SUM(CASE WHEN f.multado THEN 1 ELSE 0 END)          AS multados'), db.raw('SUM(CASE WHEN f.fechado THEN 1 ELSE 0 END)          AS fechados'), db.raw('SUM(CASE WHEN f.lacrado THEN 1 ELSE 0 END)          AS lacra
    // =============================================================================
    // 404
    // =============================================================================
    , 
