...
// Veículo
app.post('/operacoes/:id/veiculos', requireAuth, uploadFotosFields, csrfProtection, async (req, res) => {
    const user = req.session.user;
    const operacao_id = Number(req.params.id);
    const fallback = `/operacoes/${operacao_id}`;
    const go = pickReturnTo(req, fallback);
...
// Apreensão
app.post('/operacoes/:id/apreensoes', requireAuth, uploadFotosFields, csrfProtection, async (req, res) => {
    const user = req.session.user;
    const operacao_id = Number(req.params.id);
    const fallback = `/operacoes/${operacao_id}`;
    const go = pickReturnTo(req, fallback);
    ...
    const errors = [];
    if (!tipo)
        errors.push('Informe o tipo da apreensão.');
...
async (req, res) => {
    ...
    if (files.length) {
        await db('evento_fotos').insert(files.map(f => ({ evento_id: eventoId, path: `/uploads/fotos/${f.filename}` })));
    }
    return res.redirect(`/operacoes/${opId}/fiscalizacoes/${eventoId}/editar`);
});
...
app.get('/operacoes/:opId/apreensoes/:eventoId/editar', requireAuth, csrfProtection, async (req, res) => {
    ...
    const aprFotos = await db('evento_fotos')
        ...
        .orderBy('id', 'desc');
    // Lista de fiscalizações para o <select>
    const fiscListQ = db('operacao_eventos as e')
        .leftJoin('evento_fiscalizacao as f', 'f.evento_id', 'e.id')
        .where({ 'e.operacao_id': opId, 'e.tipo': 'fiscalizacao' })
        .select('e.id', db.raw(`COALESCE(NULLIF(f.nome_local,''), NULLIF(f.tipo_local,''), 'Fiscalização #' || e.id) as nome`))
        .orderBy('e.ts', 'desc');
    // (se quiser restringir à cidade do autor do evento)
    if (perm.evento?.cidade_id)
        fiscListQ.andWhere('e.cidade_id', perm.evento.cidade_id);
    const fiscList = await fiscListQ;
    // Outras apreensões da mesma fiscalização (coluna da direita)
    const aprList = await db('evento_apreensao')
...
app.get('/debug/geo/schema', requireAdminOrGestor, async (_req, res) => {
    try {
        if (DB_CLIENT === 'pg') {
            const cols = await db
            ...
            const cols = pragma?.[0]
                ?.filter((r) => ['lat', 'lng', 'accuracy'].includes(r.name))
                ?.map((r) => ({ col: r.name, type: r.type }));
...
app.get('/debug/geo/op/:id', requireAdminOrGestor, async (req, res) => {
    try {
        const opId = Number(req.params.id);
        if (!Number.isFinite(opId))
            return res.status(400).json({ error: 'opId inválido' });
        ...
        const rows = await db('operacao_eventos as e')
            ...
            .select('e.id', 'e.tipo', 'e.ts', 'e.obs', 'e.lat as e_lat', 'e.lng as e_lng', 'e.accuracy as e_acc', 'ef.lat as ef_lat', 'ef.lng as ef_lng', 'ef.accuracy as ef_acc')
            .orderBy('e.ts', 'desc')
            .limit(200);
        const stats = {
            total: rows.length,
...
app.get('/debug/geo/backfill', requireAdminOrGestor, async (_req, res) => {
    try {
        res.json(await runGeoBackfill());
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
});
app.get('/debug/geo/backfill/:id', requireAdminOrGestor, async (req, res) => {
    const opId = Number(req.params.id);
    if (!Number.isFinite(opId))
        return res.status(400).json({ error: 'id inválido' });
    try {
        res.json(await runGeoBackfill(opId));
    }
    catch (e) {
        res.status(500).json({ error: e.message });
    }
});
app.get('/debug/map/:id', requireAdminOrGestor, async (req, res) => {
    const id = Number(req.params.id);
    if (!Number.isFinite(id))
        return res.status(400).json({ error: 'id inválido' });
    const gsub = db('evento_fotos')
