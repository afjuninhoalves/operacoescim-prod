...
async function buildFiscListWithItems(filters) {
    // base das fiscalizações
    const fisc = await applyCommonWhere(db('operacao_eventos as e')
        .join('evento_fiscalizacao as f', 'f.evento_id', 'e.id')
        .leftJoin('cidades as c', 'c.id', 'e.cidade_id')
        .leftJoin('usuarios as u', 'u.id', 'e.user_id')
    ...
    const itensRows = await applyCommonWhere(db('evento_apreensao as a')
        .join('operacao_eventos as e', 'e.id', 'a.evento_id')
        .where('e.tipo', 'fiscalizacao'), filters, 'e')
        .select('a.fiscalizacao_evento_id', db.raw(`json_agg(
           json_build_object(
             'tipo', a.tipo,
    ...
    itensRows.forEach((r) => itensMap.set(Number(r.fiscalizacao_evento_id), r.itens));
    return fisc.map((r) => ({ ...r, itens: itensMap.get(Number(r.evento_id)) || [] }));
}
...
app.get('/relatorios/export.xlsx', requireAdminOrGestor, async (req, res, next) => {
    try {
        ...
        if (!f.opId)
            return res.status(400).send('Selecione uma operação.');
        ...
        (data.porCidade || []).forEach((r) => shCidade.addRow(r));
        // Fiscalizações
        ...
        (data.fiscList || []).forEach((r) => {
            const itensTxt = (r.itens || []).map((i) => {
...
app.get('/relatorios/export.pdf', requireAdminOrGestor, async (req, res, next) => {
    try {
        ...
        const [op, fiscList, porCidade] = await Promise.all([
            ...
            applyCommonWhere(db('operacao_eventos as e')
                .join('evento_fiscalizacao as f', 'f.evento_id', 'e.id')
                .leftJoin('cidades as c', 'c.id', 'e.cidade_id')
                .where('e.tipo', 'fiscalizacao'), f, 'e')
